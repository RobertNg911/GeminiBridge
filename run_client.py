...
from src.core.client import GeminiClient, BrowserStartError
...
async def main():
    ...
    try:
        ...
        while True:
            ...
            # Các lệnh không tương tác với trình duyệt
            if text.lower() == "/quit":
                break
            ...
            try:
                ...
                if response:
                    print(f"\n{C.MAGENTA}{C.BOLD}Gemini:{C.RESET}  ", end="")
                    lines = response.split("\n")
                    for i, line in enumerate(lines):
                        prefix = "           " if i > 0 else ""
                        print(f"{prefix}{C.WHITE}{line}{C.RESET}")
                else:
                    print(f"\n{C.RED}✗ Không nhận được phản hồi từ Gemini.{C.RESET}")
